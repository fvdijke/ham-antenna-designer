...
BALUN_TYPE_LABELS = {
    ...
    "stepdown_transformer_3_1": {
        "en": "Step-down transformer (3:1 turns, 9:1 impedance)",
        "nl": "Step-down transformer (3:1 wikkelingen, 9:1 impedantie)",
    },
}
...
BALUN_WHY_YAGI = {
    "en": "converts the unbalanced coax feed to the balanced driven element and suppresses common-mode current, same as on a plain dipole; the driven element's ~25 ohms needs a hairpin (beta) or gamma match to reach 50 ohms -- without it the SWR stays around 2:1",
    "nl": "zet de ongebalanceerde coaxvoeding om naar het gebalanceerde gevoede element en onderdrukt common-mode stroom, net als bij een gewone dipool; de ~25 ohm van het gevoede element vraagt een hairpin- (beta-) of gamma-aanpassing om 50 ohm te halen -- zonder blijft de SWR rond 2:1",
}
...
BALUN_WHERE_GROUND_LOOP = {
    "en": "at the feedpoint gap, where the loop's two ends connect to the step-down transformer",
    "nl": "bij de voedingsspleet, waar de twee uiteinden van de loop op de step-down transformer aansluiten",
}

BALUN_WHY_GROUND_LOOP = {
    "en": "a loop lying on the ground presents a fairly high impedance (roughly 300-1500 ohms, typically ~450 ohms) that varies with frequency and ground conditions; the 9:1 step-down transformer brings it down to the receiver's 50 ohm input",
    "nl": "een loop die op de grond ligt heeft een vrij hoge impedantie (ruwweg 300-1500 ohm, doorgaans ~450 ohm) die varieert met frequentie en bodemgesteldheid; de 9:1 step-down transformer brengt dit omlaag naar de 50 ohm ingang van de ontvanger",
}
...
BUILD_NOTES_LONGWIRE = {
    "en": {
        ...
        "warning": "NOTE: this is a receive-only, intentionally non-resonant design -- there is no single\n   \"correct\" length like a transmit antenna. {length} is a practical starting length (a quarter\n   wave at the band's low edge, kept within 10-50 m -- on LW/MW a quarter wave would be hundreds\n   of metres); longer generally improves low-frequency sensitivity. Use what space allows --\n   a shorter wire still works, just with reduced low-end response.",
    ...
    "nl": {
        ...
        "warning": "LET OP: dit is een receive-only, expres niet-resonant ontwerp -- er is geen \"juiste\"\n   lengte zoals bij een transmit-antenne. {length} is een praktische startlengte (een kwartgolf\n   op de onderkant van de band, begrensd op 10-50 m -- op LW/MW zou een kwartgolf honderden meters\n   zijn); langer verbetert over het algemeen de gevoeligheid onderin. Gebruik wat de ruimte toelaat --\n   een kortere draad werkt nog steeds, alleen met minder respons onderin.",
...
BUILD_NOTES_DISCONE = {
    "en": {
        ...
        "warning": "NOTE: published rule-of-thumb dimensions ({skirt_count} skirt wires, cone half-angle\n   {cone_angle} degrees from vertical), not an NEC-optimized design -- a solid starting point for\n   a genuinely wideband VHF/UHF receive antenna.",
        "step1": "1. Cut each cone skirt wire to {length_cone} (a free-space quarter wave at the lowest\n   frequency) and make the disc {length_disc} in diameter (0.7 x the cone's base diameter) --\n   the skirt length sets the low-frequency cutoff.",
        "step2": "2. Build the disc as a small flat ring or {skirt_count} short spokes at the top, and the cone\n   skirt as {skirt_count} wires/rods running from the apex down and outward, each at\n   about {cone_angle} degrees from vertical (cone height {height_cone}, base diameter {base_cone}).",
    ...
    "nl": {
        ...
        "warning": "LET OP: gepubliceerde vuistregel-afmetingen ({skirt_count} skirt-draden, halve tophoek van de cone\n   {cone_angle} graden t.o.v. de verticaal), geen NEC-geoptimaliseerd ontwerp -- een solide startpunt voor\n   een echt breedband VHF/UHF receive-antenne.",
        "step1": "1. Knip elke skirt-draad van de cone op {length_cone} (een kwartgolf in vrije ruimte op de laagste\n   frequentie) en maak de disc {length_disc} in diameter (0,7 x de basisdiameter van de cone) --\n   de lengte van de skirt bepaalt de onderste frequentiegrens.",
        "step2": "2. Bouw de disc als een kleine vlakke ring of {skirt_count} korte spaken aan de top, en de cone-skirt\n   als {skirt_count} draden/staven die vanaf de apex naar beneden en naar buiten lopen, elk onder\n   ongeveer {cone_angle} graden t.o.v. de verticaal (conehoogte {height_cone}, basisdiameter {base_cone}).",
...
BUILD_NOTES_GROUND_LOOP = {
    "en": {
        ...
        "warning": "NOTE: receive-only, untuned, and intentionally laid flat on the ground -- this is a\n   noise-canceling DX antenna, not a transmit design. Published field reports (KK5JY) use loops of\n   roughly 7-9 m (25-30 ft) per side; larger loops pick up more signal but also more noise.\n   {length} is this calculator's reference for your band (0.1 wavelength at the low edge, 20-60 m).",
        ...
        "step3": "3. Feedpoint: connect both loop ends across the transformer's high-impedance\n   (loop-side, 3 turns per receiver-side turn) winding. Expect roughly ~{ohms} ohms here, varying a lot with ground and frequency.",
        "step4": "4. Transformer: use a {balun_type} -- {balun_why}. Wind it on a ferrite binocular/toroid\n   core (a common choice is Fair-Rite #73 mix) with the low-impedance winding going to the coax.",
    ...
    "nl": {
        ...
        "warning": "LET OP: receive-only, ongetuned, en expres plat op de grond gelegd -- dit is een\n   ruisonderdrukkende DX-antenne, geen transmit-ontwerp. Gepubliceerde veldrapporten (KK5JY) gebruiken\n   loops van ruwweg 7-9 m per zijde; grotere loops vangen meer signaal maar ook meer ruis.\n   {length} is de referentie van deze calculator voor je band (0,1 golflengte op de onderkant, 20-60 m).",
        ...
        "step3": "3. Voedingspunt: verbind beide loop-uiteinden met de hoogohmige (loop-kant, 3 windingen per\n   winding aan ontvangerzijde) wikkeling van de transformer. Verwacht hier ruwweg ~{ohms} ohm, sterk variërend met bodem en frequentie.",
        "step4": "4. Transformer: gebruik een {balun_type} -- {balun_why}. Wikkel hem op een ferriet\n   binocular/toroide kern (Fair-Rite #73 mix is een gangbare keuze), met de laagohmige wikkeling naar de coax.",
...
BUILD_NOTES = {
    "en": {
        ...
        "step4": (
            "4. Common-mode choke: wind {choke} -- placed right at the feedpoint before the coax run back to the radio.\n"
...
WIRE_LABELS = {
    "Bare copper wire": {"en": "Bare copper wire", "nl": "Blank koperdraad"},
    "Hard drawn copper wire": {"en": "Hard drawn copper wire", "nl": "Hard getrokken koperdraad"},
    "Tinned copper wire": {"en": "Tinned copper wire", "nl": "Vertind koperdraad"},
    "Litz wire (bare)": {"en": "Litz wire (bare)", "nl": "Litzedraad (blank)"},
    "PVC installation wire": {"en": "PVC installation wire", "nl": "PVC installatiedraad"},
    "Soft stranded wire": {"en": "Soft stranded wire", "nl": "Soepel montagedraad"},
    "Speaker cable": {"en": "Speaker cable", "nl": "Luidsprekerkabel"},
    "Automotive wire": {"en": "Automotive wire", "nl": "Automotive draad"},
    "Silicone wire": {"en": "Silicone wire", "nl": "Silicon draad"},
    "PTFE/Teflon wire": {"en": "PTFE/Teflon wire", "nl": "PTFE/Teflon draad"},
    "Flexweave antenna wire": {"en": "Flexweave antenna wire", "nl": "Flexweave antennedraad"},
    "DX-Wire UL": {"en": "DX-Wire UL", "nl": "DX-Wire UL"},
    "DX-Wire Premium": {"en": "DX-Wire Premium", "nl": "DX-Wire Premium"},
    "DX-Wire Heavy Duty": {"en": "DX-Wire Heavy Duty", "nl": "DX-Wire Heavy Duty"},
    "Copperweld (uninsulated)": {"en": "Copperweld (uninsulated)", "nl": "Copperweld (ongeïsoleerd)"},
    "Copperweld (insulated)": {"en": "Copperweld (insulated)", "nl": "Copperweld (geïsoleerd)"},
    "Stainless steel antenna wire": {"en": "Stainless steel antenna wire", "nl": "RVS antennedraad"},
    "Aluminum wire": {"en": "Aluminum wire", "nl": "Aluminium draad"},
    "Military field telephone wire": {"en": "Military field telephone wire", "nl": "Militair veldtelefoondraad"},
    "CAT5 conductor": {"en": "CAT5 conductor", "nl": "CAT5 ader"},
    "CAT6 conductor": {"en": "CAT6 conductor", "nl": "CAT6 ader"},
    "Magnet wire": {"en": "Magnet wire", "nl": "Magneetdraad"},
}


# Antenna schematics (schematic.py): callouts, header, legend, viewer.
SCHEMATIC = {
    "en": {
        "title": "{label} · {band} · {freq} MHz",
        "subtitle": "Feedpoint ~{z} Ω · {balun}",
        "note": "Schematic, not to scale -- dimensions are the calculated values",
        "radiator": "Radiator",
        "radiator_len": "length {len}",
        "radiator_legs": "2 legs × {len} = {total}",
        "radiator_ocf": "{short} + {long} = {total}",
        "radiator_vertical": "vertical element, {len}",
        "loop": "Loop (radiator)",
        "loop_detail": "{n} × {side} = {total}",
        "driven": "Driven element",
        "driven_loop": "Driven loop",
        "reflector": "Reflector",
        "reflector_loop": "Reflector loop",
        "director": "Director",
        "moxon_driven": "A + 2 × B = {len}",
        "moxon_reflector": "A + 2 × D = {len}",
        "moxon_gap": "Gap C",
        "moxon_gap_detail": "{len} between the tails",
        "counterpoise": "Counterpoise",
        "cp_detail": "{len}, to the ground lug of the unun",
        "radials": "Radials",
        "radials_detail": "{n} × {len}, to the coax shield",
        "cone": "Cone (skirt)",
        "cone_detail": "{n} wires × {len}",
        "disc": "Disc",
        "disc_detail": "diameter {len}",
        "stub": "Matching stub (λ/4)",
        "stub_detail": "{len}, shorted at the bottom",
        "tap": "Feedpoint (tap)",
        "tap_detail": "{len} above the short",
        "feed_direct": "Feedpoint",
        "feed_direct_detail": "coax direct: core to disc, shield to cone",
        "box_detail": "at the feedpoint, ~{z} Ω",
        "coil_detail": "at the base, tunes to ~50 Ω",
        "tuner_detail": "in the shack, on the ladder line",
        "feed_coax": "Feed line",
        "feed_coax_detail": "{cable} to the shack",
        "feed_ladder": "Feed line: ladder line",
        "feed_ladder_detail": "open-wire line 450-600 Ω to the tuner",
        "earth": "Ground point",
        "earth_detail": "ground rod at the shack entry",
        "boom": "Boom",
        "legend_radiator": "Radiator",
        "legend_parasitic": "Parasitic element",
        "legend_stub": "Matching stub",
        "legend_counterpoise": "Counterpoise / radials",
        "legend_coax": "Feed line (coax)",
        "legend_ladder": "Feed line (ladder line)",
        "legend_box": "Balun / unun / choke",
        "legend_earth": "Ground point",
        "legend_support": "Mast / rope",
        "day": "Day",
        "night": "Night",
    },
    "nl": {
        "title": "{label} · {band} · {freq} MHz",
        "subtitle": "Voedingspunt ~{z} Ω · {balun}",
        "note": "Schematisch, niet op schaal -- maten zijn de berekende waarden",
        "radiator": "Straler",
        "radiator_len": "lengte {len}",
        "radiator_legs": "2 poten × {len} = {total}",
        "radiator_ocf": "{short} + {long} = {total}",
        "radiator_vertical": "verticaal element, {len}",
        "loop": "Lus (straler)",
        "loop_detail": "{n} × {side} = {total}",
        "driven": "Gevoed element",
        "driven_loop": "Gevoede lus",
        "reflector": "Reflector",
        "reflector_loop": "Reflectorlus",
        "director": "Director",
        "moxon_driven": "A + 2 × B = {len}",
        "moxon_reflector": "A + 2 × D = {len}",
        "moxon_gap": "Spleet C",
        "moxon_gap_detail": "{len} tussen de staarten",
        "counterpoise": "Counterpoise",
        "cp_detail": "{len}, aan de aardlip van de unun",
        "radials": "Radialen",
        "radials_detail": "{n} × {len}, aan het coaxscherm",
        "cone": "Kegel (skirt)",
        "cone_detail": "{n} draden × {len}",
        "disc": "Disc",
        "disc_detail": "diameter {len}",
        "stub": "Aanpasstub (λ/4)",
        "stub_detail": "{len}, onderaan kortgesloten",
        "tap": "Voedingspunt (aftakking)",
        "tap_detail": "{len} boven de kortsluiting",
        "feed_direct": "Voedingspunt",
        "feed_direct_detail": "coax direct: kern aan disc, scherm aan kegel",
        "box_detail": "op het voedingspunt, ~{z} Ω",
        "coil_detail": "aan de voet, stemt af op ~50 Ω",
        "tuner_detail": "in de shack, aan de ladderlijn",
        "feed_coax": "Voedingslijn",
        "feed_coax_detail": "{cable} naar de shack",
        "feed_ladder": "Voedingslijn: ladderlijn",
        "feed_ladder_detail": "open lijn 450-600 Ω naar de tuner",
        "earth": "Aardpunt",
        "earth_detail": "aardpen bij de shack-invoer",
        "boom": "Boom",
        "legend_radiator": "Straler",
        "legend_parasitic": "Parasitair element",
        "legend_stub": "Aanpasstub",
        "legend_counterpoise": "Counterpoise / radialen",
        "legend_coax": "Voedingslijn (coax)",
        "legend_ladder": "Voedingslijn (ladderlijn)",
        "legend_box": "Balun / unun / choke",
        "legend_earth": "Aardpunt",
        "legend_support": "Mast / tui",
        "day": "Dag",
        "night": "Nacht",
    },
}
