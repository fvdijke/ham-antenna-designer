...
DIM_STROKE = "#2566a8"  # dimension lines/arrows -- a distinct blue, same role
# as the cyan accent in the in-app viewer: "this is a measured distance",
# visually separate from the wire (black) and the ground (light gray).

GRID_STROKE = "#eef1f4"
GRID_SPACING = 32


def _add_arrow_marker_dim(dwg: svgwrite.Drawing):
    marker = dwg.marker(insert=(2, 2), size=(4, 4), orient="auto", id="dimarrow")
    marker.add(dwg.path(d="M0,0 L4,2 L0,4 Z", fill=DIM_STROKE))
    dwg.defs.add(marker)


def _draw_grid(dwg: svgwrite.Drawing, width: float, height: float):
    """Faint graph-paper grid behind the drawing -- reads as a proper
    engineering schematic instead of a bare white page."""
    for x in range(0, int(width) + 1, GRID_SPACING):
        dwg.add(dwg.line(start=(x, 0), end=(x, height), stroke=GRID_STROKE, stroke_width=0.5))
    for y in range(0, int(height) + 1, GRID_SPACING):
        dwg.add(dwg.line(start=(0, y), end=(width, y), stroke=GRID_STROKE, stroke_width=0.5))


def _draw_frame(dwg: svgwrite.Drawing, width: float, height: float):
    """Corner-bracket frame matching the in-app viewer's HUD-style border."""
    bracket = 18
    inset = 4
    pts = [(inset, inset), (width - inset, inset), (width - inset, height - inset), (inset, height - inset)]
    for cx, cy in pts:
        dx = 1 if cx < width / 2 else -1
        dy = 1 if cy < height / 2 else -1
        dwg.add(dwg.line(start=(cx, cy), end=(cx + bracket * dx, cy), stroke="black", stroke_width=1.2))
        dwg.add(dwg.line(start=(cx, cy), end=(cx, cy + bracket * dy), stroke="black", stroke_width=1.2))


def _rounded_rect(dwg, x, y, w, h, fill, stroke, stroke_width):
    return dwg.rect(insert=(x, y), size=(w, h), rx=4, ry=4, fill=fill, stroke=stroke, stroke_width=stroke_width)


def _svg_from_scene(scene: Scene) -> svgwrite.Drawing:
    dwg = _new_drawing(scene.width, scene.height)
    _add_arrow_marker_dim(dwg)
    _draw_grid(dwg, scene.width, scene.height)
    ...
    for x1, y1, x2, y2 in scene.dim_lines:
        dwg.add(dwg.line(start=(x1, y1), end=(x2, y2), stroke=DIM_STROKE, stroke_width=0.6,
                          marker_start="url(#dimarrow)", marker_end="url(#dimarrow)"))
    ...
    for feed_x, feed_y, box_x, box_y, text in scene.balun_boxes:
        ...
        box_h = 7.0 * 1.8
        dwg.add(_rounded_rect(dwg, box_x - 4, box_y - box_h / 2, box_w, box_h, fill="white", stroke="black", stroke_width=0.75))
    ...
    for anchor_x, anchor_y, box_x, box_y, text, kind in scene.tag_boxes:
        ...
        box_w = len(text) * 7.0 * 0.62 + 10
        box_h = 7.0 * 1.8
        dwg.add(_rounded_rect(dwg, box_x - 4, box_y - box_h / 2, box_w, box_h, fill="white", stroke=stroke, stroke_width=1.0))
        dwg.add(dwg.text(text, insert=(box_x, box_y + 2.5), font_size="7", font_family="sans-serif", font_weight="bold"))

    _draw_frame(dwg, scene.width, scene.height)
    return dwg
